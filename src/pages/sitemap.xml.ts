...
// The living map is built for every language and was absent from the sitemap.
const mapPaths = SUPPORTED_LANGS.map((lang) => `/${lang}/map`);
const seoPaths = SUPPORTED_LANGS.flatMap((lang) =>
  getSeoPagePaths().map((page) => `/${lang}${page.path}`),
);

const urls = ["/", ...homePaths, ...mapPaths, ...seoPaths];
